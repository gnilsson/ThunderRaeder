...
namespace ThunderRaeder.Client.Authentication
{
    public class ApiAuthenticationStateProvider : AuthenticationStateProvider
    {
        ...
        {
            var savedToken = await _localStorage.GetItemAsync<string>(AuthDescriptions.TokenKey);

            if (string.IsNullOrWhiteSpace(savedToken))
                return new AuthenticationState(new ClaimsPrincipal(new ClaimsIdentity()));

            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(AuthDescriptions.Bearer, savedToken);
